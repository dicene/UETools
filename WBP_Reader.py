...
class Window(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("WBP Reader")
        ...
        self.normal_tab = QtWidgets.QVBoxLayout(self)
        self.normal_tab_widget = QtWidgets.QWidget(self)
        self.normal_tab_widget.setLayout(self.normal_tab)
        self.tab_widget.addTab(self.normal_tab_widget, "Normal")

        self.raw_tab = QtWidgets.QTextEdit(self)
        ...
        self.normal_tab.addLayout(self.name_block)
        ...
        self.normal_tab.addLayout(self.cdo_block)
        self.cdo_text = QtWidgets.QLabel(self)
        self.normal_tab.addWidget(self.cdo_text)

        self.function_list_label = QtWidgets.QLabel(self)
        self.function_list_label.setText("Functions:")
        self.normal_tab.addWidget(self.function_list_label)
        self.function_list = QtWidgets.QListWidget(self)
        self.normal_tab.addWidget(self.function_list)
        ...
        self.normal_tab.addWidget(self.bp_tree)
        ...
        self.raw_tab.textChanged.connect(self.raw_text_changed)

        self.load_settings()
        if self.path_edit.text() != "":
            self.load_file(self.path_edit.text())
        self.load_blueprint(self.raw_tab.toPlainText())
    ...
    def raw_text_changed(self):
        print(f"Text changed: {self.raw_tab.toPlainText()}")
    ...
    def load_file(self, file_path):
        with open(file_path, 'r') as file:
            self.raw_tab.setText(file.read())
            file.seek(0)
            self.load_blueprint(file.read())

    def load_blueprint(self, text):
        print(f"Text: {text}")
